...
#Returns True if current token still works. Returns False if it expired or does not exist.
...
#returns page to generate playlist
@app.route('/ai_playlist_generator')
def ai_playlist_generator():
    return render_template('index.html', token = session['access_token'])

#Takes user input and uses it to create array of song names using gemini. Also Creates Playlist
...
def generate_playlist():
    ...
    if len(playlist) != 1:    
        ...
        data = {
            "name": playlist_name,
            "description": "A playlist generated by Gemini in response to: \""+user_input+"\"."
        }
        ...
        if playlist_response.status_code==201:
            ...
            return redirect('/add_songs_to_playlist')
        else:
...
#takes list of song names and adds them to a playlist
@app.route('/add_songs_to_playlist')    
def add_songs():
...
#takes generated playlist and displays the name and cover
...
def generation_complete():
    ...
    return render_template('gen_result.html', titles=song_titles, covers=song_cover_urls)

#prompts user to try again if gemini gives unexpected response
